...
# CONFIG
# -----------------------------
MODEL_NAME = "RA-ALTA/pl-en-intermediate"
MAX_LENGTH = 512
IGNORE_INDEX = -100
HF_REPO = "RA-ALTA/pl-en-intermediate-alpaca-english-L1"
...
alpaca_polish = load_dataset("saillab/alpaca-polish-cleaned", split="train")

print(alpaca_english.column_names)
print(alpaca_polish.column_names)

print(len(alpaca_english))
print(len(alpaca_polish))
...
alpaca_polish = alpaca_polish.map(normalize_example)

# remove the rows with input:
# Keep only rows where input is empty
alpaca_polish = alpaca_polish.filter(
...
# rename dataset columns:
alpaca_polish = alpaca_polish.rename_columns({
    "instruction": "instruction",
    "output": "response",
})
...
alpaca_polish = alpaca_polish.shuffle(seed=42)

min_size = min(len(alpaca_english), len(alpaca_polish))
print(min_size)
...
alpaca_polish_small = alpaca_polish.select(range(target_size))

print(len(alpaca_english_small))
print(len(alpaca_polish_small))


dataset = concatenate_datasets([alpaca_english_small, alpaca_polish_small])
...
sum(1 for x in alpaca_english if x["response"].strip() == "")
