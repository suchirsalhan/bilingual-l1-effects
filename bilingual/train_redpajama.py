...
# ============================================================
# STREAMING DATA WITH PERSISTENT HTTP (FIXED)
# ============================================================
...
def streaming_blocks(dataset_name, config, split,
    ...
    # Use HuggingFace dataset for L1
    ...
    # RedPajama streaming with persistent session
            ...
            with gzip.GzipFile(fileobj=r.raw) as f:
                for line in f:
                    ...
                    if not text or not gopher_rules_pass(doc):
                        continue
                    ids = tokenizer(text, truncation=False, padding=False, add_special_tokens=False)["input_ids"]
                    if seen + len(ids) < skip_tokens:
                        seen += len(ids)
                        continue
                    buffer.extend(ids)
...
def train(lang):
    ...
    # L1 stream
    l1_stream = streaming_blocks("uonlp/CulturaX", lang, "train",
                                 global_rank, world_size, skip_tokens=state["l1_seen"])
    # L2 RedPajama stream
    l2_stream = streaming_blocks("togethercomputer/RedPajama-Data-V2",
                                 dict(name="default", partition="head_middle", snapshots=["2023-06"], languages=["en"]),
                                 "train", global_rank, world_size, skip_tokens=state["l2_seen"])
    ...
    if state["phase"] == 1:
        ...
        while state["l1_seen"] < PHASE1_TOKENS:
            ...
            if is_master and step_idx % 50 == 0:
                ...
                logging.info(f"step {step_idx} | loss {loss:.3f} | L1 {state['l1_seen']/1e9:.2f}B | {state['l1_seen']/elapsed/1e6:.1f}M tok/s")
        state["phase"] = 2
    ...
    while state["l2_seen"] < L2_TOTAL:
        for stream_name in ["l1", "l2", "l2"]:
            ...
            if stream_name == "l2":
                ...
                state["l1_seen"] += tokens
            if is_master and step_idx % 50 == 0:
                logging.info(f"step {step_idx} | loss {loss:.3f} | L2 {state['l2_seen']/1e9:.2f}B | ratio {state['l1_seen']/max(1,state['l2_seen']):.2f}")
