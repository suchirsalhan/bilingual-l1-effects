...
# ================= TOKENIZER (FULLY STREAMING) =================
def sentence_iterator(lang):
    """Generator over sentences from CulturaX and RedPajama datasets."""
    ds1 = load_dataset("uonlp/CulturaX", lang, split="train", streaming=True)
    for ex in ds1:
        text = ex.get("text")
        if text: yield text.replace("\n", " ")

    # RedPajama head-middle snapshot
...
def train_sentencepiece_streaming(lang, is_ddp, global_rank):
    ...
    if global_rank == 0:
        ...
        spm.SentencePieceTrainer.train(
            sentence_iterator=sentence_iterator(lang),
...
def streaming_blocks(dataset, config, rank, world_size, tokenizer, skip_tokens=0):
    """Streaming blocks for CulturaX or RedPajama"""
...
def train(lang):
    ...
    l2_iter = ResilientStreamingIterator("togethercomputer/RedPajama-Data-V2",
    ...
    while state["l2_seen"] < L2_TOTAL:
        for src in ["l1", "l2", "l2"]:
            ...
            # Checkpoints
            if state["checkpoint_idx"] < len(checkpoint_tokens) and state["l2_seen"] >= checkpoint_tokens[state["checkpoint_idx"]]:
