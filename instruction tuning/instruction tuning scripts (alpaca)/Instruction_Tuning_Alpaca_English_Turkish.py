...
# CONFIG
# -----------------------------
MODEL_NAME = "RA-ALTA/tr-en-advanced"
MAX_LENGTH = 512
IGNORE_INDEX = -100
HF_REPO = "RA-ALTA/tr-en-advanced-alpaca-english-L1"
...
alpaca_turkish = load_dataset("TFLai/Turkish-Alpaca", split="train")

print(alpaca_english.column_names)
print(alpaca_turkish.column_names)

print(len(alpaca_english))
print(len(alpaca_turkish))
...
alpaca_turkish = alpaca_turkish.map(normalize_example)

# remove the rows with input:
# Keep only rows where input is empty
alpaca_turkish = alpaca_turkish.filter(
...
# rename dataset columns:
alpaca_turkish = alpaca_turkish.rename_columns({
    "instruction": "instruction",
    "output": "response",
})
...
alpaca_turkish = alpaca_turkish.shuffle(seed=42)

min_size = min(len(alpaca_english), len(alpaca_turkish))
print(min_size)
...
alpaca_turkish_small = alpaca_turkish.select(range(target_size))

print(len(alpaca_english_small))
print(len(alpaca_turkish_small))
...
dataset = concatenate_datasets([alpaca_english_small, alpaca_turkish_small])
