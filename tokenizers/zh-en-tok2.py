...
LANG = "zh"
VOCAB_SIZE = 50_000
BOOTSTRAP_SENTENCES = 500_000  # smaller for speed
...
def cache_training_corpus():
    ...
    with open(CORPUS_FILE, "w", encoding="utf-8") as f:
        for _ in range(BOOTSTRAP_SENTENCES // 2):
            try:
                f.write(next(zh_iter)["text"].replace("\n", " ") + "\n")
                f.write(next(en_iter)["text"].replace("\n", " ") + "\n")
            except StopIteration:
                break
    print("✅ Corpus caching complete.")
...
def train_and_push():
    ...
    print(f"🚀 Training Byte-Level BPE tokenizer (50k vocab)...")
