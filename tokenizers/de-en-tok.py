...
def get_training_corpus():
    de_ds = load_dataset("uonlp/CulturaX", "de", split="train", streaming=True)
    en_ds = load_dataset("HuggingFaceFW/fineweb-edu", split="train", streaming=True)
    
    de_iter, en_iter = iter(de_ds), iter(en_ds)
    for _ in range(BOOTSTRAP_SENTENCES // 2):
        try:
            yield next(de_iter)["text"].replace("\n", " ")
...
def train_and_push():
    ...
    trainer = trainers.BpeTrainer(
        ...
        # We must initialize the alphabet with all 256 possible bytes
...
def run_benchmark():
    ...
    test_cases = {
        "DE_Legal": "Hallo, ich bin glucklich.",
