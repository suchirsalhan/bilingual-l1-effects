...
def get_training_corpus():
    tr_ds = load_dataset("uonlp/CulturaX", "tr", split="train", streaming=True)
    en_ds = load_dataset("HuggingFaceFW/fineweb-edu", split="train", streaming=True)
    
    tr_iter, en_iter = iter(tr_ds), iter(en_ds)
    for _ in range(BOOTSTRAP_SENTENCES // 2):
        try:
            yield next(tr_iter)["text"].replace("\n", " ")
